...
@auth_router.post(
    "/token", status_code=status.HTTP_200_OK, responses=POST_TOKEN_RESPONSES
)
...
async def authenticate_user(data: OAuth2PasswordRequestForm = Depends()):
    try:
        async with get_session() as db_session:
            ...
            response = await dals.get_token_user(
            ...
            return JSONResponse(
                content={
                    **ResponseType(
                        loc=[],
                        msg="You have successfully got a session in the Deepmode service!",
                        reason="",
                        type="success",
                        input=response.model_dump(),
                    ).model_dump(),
...
@auth_router.post(
    ...
    responses=POST_TOKEN_REFRESH_RESPONSE,
)
@exeption_handling_decorator
async def authenticate_refresh_token(refresh_token: str = Header()):
    try:
        async with get_session() as db_session:
            ...
            return ResponseAPI(
                msg="You have successfully updated the session in the Deepmode service!",
                reason="",
                input=response.model_dump(),
                status_code=status.HTTP_200_OK,
            )

...
class TimeManager:
    ...
    def __init__(cls, date: Optional[RequestDateType] = None, isReturnTimestamp: bool = False):
        cls.isReturnTimestamp = isReturnTimestamp
        if date is not None:
            cls.setNewDate(date=date)

    def format_to_datetime(cls, withUtc: bool = False) -> str:
        return cls.date.format("YYYY-MM-DD HH:mm:ss Z") if withUtc else cls.date.to_datetime_string()

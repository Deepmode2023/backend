...
def upgrade() -> None:
    ...
    op.create_table('spaced_repetition',
                    ...
                    sa.Column('date_repitition', sa.DateTime(
                        timezone=True), nullable=False),
                    sa.Column('date_last_repetition', sa.DateTime(
                        timezone=True), nullable=False),
                    sa.ForeignKeyConstraint(['user_id'], ['users.user_id'], ),
                    sa.PrimaryKeyConstraint('id')
                    )
    op.drop_constraint('shared_preference_user_id_fkey',
                       'shared_preference', type_='foreignkey')
    op.create_foreign_key(None, 'shared_preference', 'users', [
                          'user_id'], ['user_id'], ondelete='CASCADE')
    op.drop_constraint('words_user_id_fkey', 'words', type_='foreignkey')
    op.create_foreign_key(None, 'words', 'users', ['user_id'], [
...
def downgrade() -> None:
    ...
    op.create_foreign_key('words_user_id_fkey', 'words',
                          'users', ['user_id'], ['user_id'])
    op.drop_constraint(None, 'shared_preference', type_='foreignkey')
    op.create_foreign_key('shared_preference_user_id_fkey',
                          'shared_preference', 'users', ['user_id'], ['user_id'])
